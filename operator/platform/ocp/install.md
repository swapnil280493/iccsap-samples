# Installing IBM Content Collector for SAP ga-4.0.0.4 on Red Hat OpenShift
...
1. Go to the prerequisites page in the [IBM Content Collector for SAP ga-4.0.0.4](https://www.ibm.com/support/knowledgecenter/SSRW2R_4.0.0/doc/container/tsk_preparing.html) Knowledge Center.
...
```bash
$ oc create secret docker-registry admin.registrykey --docker-server=<registry_url> --docker-username=<your_account> --docker-password=<your_password> --docker-email=iccsaptest@ibm.com
```
...
Use the customized `iccsap_v1_iccsap_cr_template.yaml` file that you updated for your software and environment, and run the following commands to deploy the configured components:

   ```bash
   $ cat descriptors/my_iccsap_v1_iccsap_cr.yaml
   // check that all the components you want to install are configured
   $ oc apply -f descriptors/my_iccsap_v1_iccsap_cr.yaml -n <namespace>
   ```
